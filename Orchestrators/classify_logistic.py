...
model_p = mp.ModelParameters(input_feature_count=features, layers=[1], activations=["softmax"], outputclasses=num_classes)
opts = [
...
for i, opt in enumerate(opts):
    with tf.Session() as sess:
        sess.run(init1)
        # self, session, model_parameters, number_of_epochs, optimizer, minibatch_size=100
